...
def create_dashboard(server):
    ...
    # Load DataFrame
    url = "https://raw.githubusercontent.com/nantesmetropole/school_meal_forecast_regressions/main/data/code_etablissement.csv"
    ...
    children=[
        html.Div(
            id="header",
            children=[
                html.A("Home", className="link-button",
                    href="/",
                ),
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    id="graph-container",
                    children=[
                        html.P(id="chart-selector", children="Choix du graph:"),
                        dcc.Dropdown(
                            options=[
                                {
                                    "label": "Graph 1",
    ...
    @dash_app.callback(
        Output("cantines_nantes", "figure"),
        [Input("years-slider", "value")],
        [State("cantines_nantes", "figure")],
    )
    def display_map(year, figure):
        ...
        data = [
            dict(
                lat=df["Coordonnee X"],
                lon=df["Coordonnee Y"],
                text=df["Dénomination principale"],
                type="scattermapbox",
                hoverinfo="text",
    ...
    @dash_app.callback(Output("heatmap-title", "children"), [Input("years-slider", "value")])
    ...
    def display_selected_data(selectedData, chart_dropdown, year):
        ...
        for i in range(len(fips)):
            if len(fips[i]) == 4:
                fips[i] = "0" + fips[i]
        dff = df[df["Code état établissement"].isin(fips)]
